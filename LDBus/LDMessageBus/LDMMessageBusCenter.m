    ...
    if (receiveMessageConfigurationList && receiveMessageConfigurationList.count > 0) {
        ...
        for (int i = 0; i < receiveMessageConfigurationList.count; i++) {
            LDMReceiveMessageConfigurationItem *receiveMessageConfigurationItem =
                [receiveMessageConfigurationList objectAtIndex:i];
#ifdef DEBUG
            assert(receiveMessageConfigurationItem.messageCode != nil &&
                   ![receiveMessageConfigurationItem.messageCode isEqualToString:@""]);
#endif
            NSMutableArray *oneReceiverList =
                [_messageReceiveItemList objectForKey:receiveMessageConfigurationItem.messageCode];
            //如果为设置，初始化一个接收者池子
            if (oneReceiverList == nil) {
                oneReceiverList = [[NSMutableArray alloc] initWithCapacity:2];
                [_messageReceiveItemList setObject:oneReceiverList
                                            forKey:receiveMessageConfigurationItem.messageCode];
            }

//检查handler并且实例化
#ifdef DEBUG
            assert(receiveMessageConfigurationItem.receiveObjectString != nil &&
                   ![receiveMessageConfigurationItem.receiveObjectString isEqualToString:@""]);
#endif
            Class receiveObjectClass =
                NSClassFromString(receiveMessageConfigurationItem.receiveObjectString);
            if (receiveObjectClass != nil &&
                ...
                NSAssert(NO, @"messageReceiver: %@ has invalid object receiver (%@) in message bus",
                         receiveMessageConfigurationItem.messageName,
                         receiveMessageConfigurationItem.receiveObjectString);
#endif
            }
    ...
    if (postMessageConfigurationList && postMessageConfigurationList.count > 0) {
        for (int i = 0; i < postMessageConfigurationList.count; i++) {
            LDMPostMessageConfigurationItem *postMessageConfigurationItem =
                [postMessageConfigurationList objectAtIndex:i];
            //在parse的时候已经确保没有空参数传入
            [self registerListeningNotification:postMessageConfigurationItem.messageName
                                           code:postMessageConfigurationItem.messageCode];
        }
    }
}
...
+ (void)postMessage:(NSString *)message userInfo:(NSDictionary *)aUserInfo
{
    NSNotification *notification =
        [NSNotification notificationWithName:message object:nil userInfo:aUserInfo];
    [self postNotification:notification];
}
