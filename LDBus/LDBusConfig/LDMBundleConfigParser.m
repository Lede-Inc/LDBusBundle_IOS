         ...
         attributes:(NSDictionary *)attributeDict
{
    ...
    //标记是否有url_handler_list,如果存在，才开始解析ViewController
        ...
        if (mark_url_handler_list != nil) {
            ...
            if (_configurationItem.urlViewCtrlConfigurationList == nil) {
                _configurationItem.urlViewCtrlConfigurationList =
                    [[NSMutableArray alloc] initWithCapacity:2];
            }
            // check
            [self parser:parser
                         attributes:attributeDict
                parseEmptyKeyParams:@[ @"name", @"class" ]
                          nameSpace:elementName];
            NSString *viewCtrlName = IFNIL(attributeDict[@"name"]);
            if (_configurationItem && viewCtrlName && ![viewCtrlName isEqualToString:@""]) {
                LDMURLViewCtrlConfigurationItem *urlViewCtrlConfigurationItem = [
                    [LDMURLViewCtrlConfigurationItem alloc]
                    initWithURLViewCtrlConfigurationItem:viewCtrlName
                                                   class:IFNIL(attributeDict[@"class"])
                                                 webPath:IFNIL(attributeDict[@"webpath"])
                                                webQuery:[IFNIL(attributeDict[@"webquery"])
                                                             stringByReplacingOccurrencesOfString:
                                                                 @";" withString:@"&"]
                                                  parent:IFNIL(attributeDict[@"parent"])
                                                    type:[self getTypeFromString:
                                                                   IFNIL(attributeDict[@"type"])]];
                [_configurationItem.urlViewCtrlConfigurationList
                    addObject:urlViewCtrlConfigurationItem];
            }
        }
    }
        ...
        if (mark_url_handler_list && mark_url_handler_viewcontroller) {
            ...
            if (_configurationItem && _configurationItem.urlViewCtrlConfigurationList &&
                urlPatternName) {
                LDMURLViewCtrlConfigurationItem *last =
                    [_configurationItem.urlViewCtrlConfigurationList lastObject];
                if (last.urlViewCtrlPatternConfigurationList == nil) {
                    last.urlViewCtrlPatternConfigurationList =
                        [[NSMutableArray alloc] initWithCapacity:2];
                }
                LDMURLViewCtrlPatternConfigurationItem *urlViewCtrlPatternConfigurationItem = [
                    [LDMURLViewCtrlPatternConfigurationItem alloc]
                    initWithURLViewCtrlPatternConfigurationItem:
                        IFNIL(attributeDict[
                            @"name"]) query:[IFNIL(attributeDict[@"webquery"])
                                                stringByReplacingOccurrencesOfString:@";"
                                                                          withString:@"&"]
                                                      fragement:IFNIL(attributeDict[@"webfrage"])
                                                           type:[self getTypeFromString:
                                                                          IFNIL(attributeDict[
                                                                              @"type"])]
                                                         parent:IFNIL(attributeDict[@"parent"])];
                [last.urlViewCtrlPatternConfigurationList
                    addObject:urlViewCtrlPatternConfigurationItem];
            }
        }
    }
    ...
    //标记是否有service_list, 如果存在，才开始解析service列表
        ...
        if (mark_service_list != nil) {
            ...
            [self parser:parser
                         attributes:attributeDict
                parseEmptyKeyParams:@[ @"name", @"class", @"protocol" ]
                          nameSpace:elementName];
            if (_configurationItem.serviceConfigurationList == nil) {
                _configurationItem.serviceConfigurationList =
                    [[NSMutableArray alloc] initWithCapacity:2];
            }
            LDMServiceConfigurationItem *serviceConfigurationItem =
                [[LDMServiceConfigurationItem alloc]
                    initWithServiceConfigurationItem:IFNIL(attributeDict[@"name"])
                                            protocol:IFNIL(attributeDict[@"protocol"])
                                               class:IFNIL(attributeDict[@"class"])];
    ...
    //标记是否有post, 如果存在，才开始解析service列表
        ...
        if (mark_post_message_list != nil) {
            ...
            [self parser:parser
                         attributes:attributeDict
                parseEmptyKeyParams:@[ @"name", @"code" ]
                          nameSpace:elementName];
            if (_configurationItem.postMessageConfigurationList == nil) {
                _configurationItem.postMessageConfigurationList =
                    [[NSMutableArray alloc] initWithCapacity:2];
            }

            LDMPostMessageConfigurationItem *postMessageConfigurationItem =
                [[LDMPostMessageConfigurationItem alloc]
                    initWithPostMessageConfigurationItem:IFNIL(attributeDict[@"name"])
                                                    code:IFNIL(attributeDict[@"code"])];
            [_configurationItem.postMessageConfigurationList
                addObject:postMessageConfigurationItem];
        }
    }
    ...
    //标记是否有receive_message_list, 如果存在，才开始解析service列表
        ...
        if (mark_receive_message_list != nil) {
            ...
            [self parser:parser
                         attributes:attributeDict
                parseEmptyKeyParams:@[ @"code", @"class" ]
                          nameSpace:elementName];
            if (_configurationItem.receiveMessageConfigurationList == nil) {
                _configurationItem.receiveMessageConfigurationList =
                    [[NSMutableArray alloc] initWithCapacity:2];
            }

            LDMReceiveMessageConfigurationItem *receiveMessageConfigurationItem =
                [[LDMReceiveMessageConfigurationItem alloc]
                    initWithReceiveMessageConfigurationItem:IFNIL(attributeDict[@"name"])
                                                       code:IFNIL(attributeDict[@"code"])
                                              receiveObject:IFNIL(attributeDict[@"class"])];
            [_configurationItem.receiveMessageConfigurationList
                addObject:receiveMessageConfigurationItem];
        }
    }
}
...
- (void)parser:(NSXMLParser *)parser
             attributes:(NSDictionary *)attributeDict
    parseEmptyKeyParams:(NSArray *)keyParams
    ...
    if (keyParams && keyParams.count > 0) {
        for (int i = 0; i < keyParams.count; i++) {
            ...
            if (keyParamValue == nil || [keyParamValue isEqualToString:@""]) {
                NSAssert(NO, @"parse %@'s param %@ is empty, location: line %ld col %ld", nameSpace,
                         keyParam, (long)[parser lineNumber], (long)[parser columnNumber]);
            }
        }  // for
    }
#endif
...
{
#ifdef DEBUG
    NSAssert(NO, @"xml parse error line %ld col %ld", (long)[parser lineNumber],
