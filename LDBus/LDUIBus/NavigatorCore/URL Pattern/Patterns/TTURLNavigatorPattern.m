...
{
    return (nil != _targetObject && [_targetObject class] != _targetObject) || nil != _targetClass;
}
...
- (void)analyzeMethod
{
    ...
    Method method = [self callsInstanceMethod] ? class_getInstanceMethod(cls, _selector)
               ...
               argNames:(NSArray *)argNames
{
    if ([pattern isKindOfClass:[TTURLWildcard class]]) {
        ...
        if (wildcard.argIndex == NSNotFound) {
            TTDINFO(@"Argument %@ not found in @selector(%s)", wildcard.name,
