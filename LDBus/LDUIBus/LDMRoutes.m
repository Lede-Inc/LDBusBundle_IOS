...
- (BOOL)isParametersMatchForURL:(NSURL *)URL components:(NSArray *)URLComponents
{
    ...
    if (!self.patternPathComponents) {
        self.patternPathComponents = [[self.pattern pathComponents]
            filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"NOT SELF like '/'"]];
    }
    ...
    BOOL routeContainsWildcard =
        !NSEqualRanges([self.pattern rangeOfString:@"*"], NSMakeRange(NSNotFound, 0));
    if (componentCountEqual || routeContainsWildcard) {
        // now that we've identified a possible match, move component by component to check if it's
        // a match
        ...
        for (NSString *patternComponent in self.patternPathComponents) {
            ...
            if ([patternComponent hasPrefix:@":"]) {
                ...
                // a non-variable component did not match, so this route doesn't match up - on to
                // the next one
                isParameterMatch = NO;
                break;
            }
         ...
         isModal:(BOOL)isModal
{
    ...
    if (!webCtrlClass ||
        ![webCtrlClass conformsToProtocol:@protocol(LDMBusWebControllerProtocol)]) {
#ifdef DEBUG
        NSAssert(NO, @"the specified web controller is not exist in bundle or not conform to "
                     @"LDMBusWebControllerProtocol, please check!!!");
#endif
    }
...
+ (BOOL)routeURL:(NSURL *)URL
  withController:(LDMRoutes *)routesController
    executeBlock:(BOOL)executeBlock
{
    BOOL didRoute = NO;
    ...
    for (_LDMRoute *route in routes) {
        ...
        if (isMatch) {
            ...
            id webControllerHandler =
                [[[NSClassFromString(route.webHandlerClassString) class] alloc] init];
            if (webControllerHandler &&
                [webControllerHandler isKindOfClass:[UIViewController class]]) {
                ...
                [_navigator presentDependantController:webControllerHandler
                                      parentController:_navigator.topViewController
                                                  mode:(route.isModal ? TTNavigationModeModal
                                                                      : TTNavigationModeCreate)
