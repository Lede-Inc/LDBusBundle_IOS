...
- (TTURLActionResponse *)handleURLActionRequest:(TTURLAction *)action
{
    ...
    TTURLNavigatorPattern *pattern =
        [_bundleMap matchObjectPattern:[NSURL URLWithString:action.urlPath]];
    TTURLActionResponse *actionResponse =
        [[TTURLActionResponse alloc] initWithViewController:controller
                                                    pattern:pattern
                                               sourceBundle:_bundleName];
                                   ...
                                   pattern:(TTURLPattern **)pattern
{
    ...
    if (fragmentRange.location != NSNotFound) {
        ...
        //如果该URL能够处理，那么BaseURL一定是在该bundleMap下
            ...
            id object = [_bundleMap objectForURL:baseURL
                                           query:nil
...
- (UIViewController *)viewControllerForAction:(TTURLAction *)action
{
    ...
    UIViewController *controller =
        [self viewControllerForURL:urlPath query:action.query pattern:&pattern];
    return controller;
}
...
- (BOOL)createAndPresentViewControllerForAction:(TTURLAction *)action
{
    ...
    if ([self presentViewController:controller navigator:_navigator action:action]) {
        success = YES;
    }
