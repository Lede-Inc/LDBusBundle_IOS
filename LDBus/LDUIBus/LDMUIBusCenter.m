            ...
            if ([bundle.uibusConnetor canOpenInBundle:action.urlPath]) {
                @synchronized(_UIBusMessageQueue)
                {
                    ...
                    //如果只是url实例化，则调用connetor返回
                        ...
                        TTURLActionResponse *response =
                            [bundle.uibusConnetor handleURLActionRequest:action];
                        if (response.viewController) {
                            success = YES;
                            //存储结果
                            [_UIBusMessageQueue replaceObjectAtIndex:_UIBusMessageQueue.count - 1
                                                          withObject:@{
                                                              TITLE_MESSAGEACTION : action,
                                                              TITLE_MESSAGERESULT : response
                                                          }];
                        }
                    }
                }
...
{
    [[self class] registerSpecialScheme:scheme
                              addRoutes:routePattern
                       handleController:handleControllerClassString
                                isModal:YES];
}
...
{
    if (scheme && ![scheme isEqualToString:@""] && routePattern &&
        ![routePattern isEqualToString:@""] && handleControllerClassString &&
        ![handleControllerClassString isEqualToString:@""]) {
        [[LDMRoutes routesForScheme:scheme] addRoute:routePattern
                                          webHandler:handleControllerClassString
                                             isModal:isModal];
    }
}
