...
- (id)initWithType:(int)type query:(NSDictionary *)query
{
    ...
    if (self) {
        ...
        for (int j = 0; j < keys.count; j++) {
            NSLog(@">>>key=%@:value=%@", [keys objectAtIndex:j],
                  [dic objectForKey:[keys objectAtIndex:j]]);
        }
    }
    return self;
}
...
- (id)initWithFood:(NSString *)food
{
    if (self = [super init]) {
        ...
        self.title = food;
        self.navigationItem.rightBarButtonItem =
            [[UIBarButtonItem alloc] initWithTitle:@"Nutrition"
                                             style:UIBarButtonItemStyleBordered
                                            target:self
                                            action:@selector(showNutrition)];
    }
    return self;
}
...
- (id)initWithModeController:(NSString *)food
{
    if (self = [super init]) {
        ...
        self.title = food;
        self.navigationItem.rightBarButtonItem =
            [[UIBarButtonItem alloc] initWithTitle:@"dissmiss"
                                             style:UIBarButtonItemStyleBordered
                                            target:self
                                            action:@selector(dismiss)];
    }
...
- (id)initWithAbout:(NSString *)about
{
    if (self = [super init]) {
        ...
        self.content = about;
        self.text =
            [NSString stringWithFormat:@"<b>%@</b> is the name of this page.  Exciting.", about];
...
- (void)loadView
{
    ...
    if (_contentType == ContentTypeNutrition) {
        ...
        [button addTarget:@"tt://order/food"
