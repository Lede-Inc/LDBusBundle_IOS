...
- (void)viewDidLoad
{
    ...
    _tableView =
        [[UITableView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, self.view.frame.size.width,
...
- (void)setPage:(MenuPage)page
{
    ...
    if (_page == MenuPageBreakfast) {
        [_keyArrary addObjectsFromArray:@[
            @"点击push view，push:netescaipiao://food",
            @"push view",
        ...
        if (_page == 1) {
            if (indexPath.row == 3) {
                ...
                [LDMBusContext openURL:url
                                 query:@{
                                     @"_array_" : @[ @"hello", @"baby" ],
                                     @"_dic_" : @{@"hello" : @"1", @"baby" : @"2"}
                                 }];
            } else if (indexPath.row == 9) {
                NSNotification *notification =
                    [NSNotification notificationWithName:url
                                                  object:@"i am a object"
                                                userInfo:@{
                                                    @"userInfo" : @"i am form userinfo"
                                                }];
